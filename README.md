...
The graphical apps are still compatibility smoke tests rather than full visual conformance tests. Rendering now includes the maintained window model, `PutImage` pixel data, XRender fills/composites/glyphs, painted pixmap/offscreen surfaces in SVG previews, and automated Xvfb-vs-Kotlin Robot parity coverage for deterministic Java2D/AWT primitives, overlapping Swing windows, and owned popup windows. The single-window primitive probe also compares the Kotlin server's SVG-exported framebuffer against the Xvfb reference. More X drawing semantics are still pending, especially GL/JCEF-backed paths and broader app-managed surface presentation.
