...
The parity probe writes PNG/SVG/HTML/text diagnostics, SVG layer and HTML per-window preview inventories, visual diffs/metrics, visual region metrics, and IntelliJ client logs under `build/tmp/intellij-community-smoke/` so visual drift can be inspected without rerunning the heavyweight clients immediately.
