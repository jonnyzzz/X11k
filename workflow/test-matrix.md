...
Build both Docker images with `scripts/run-gradle-bounded.sh dockerBuildX11Images`, then run the rough VSCode parity probe explicitly with `-Dx.vscodeParity=true`; it writes Xvfb reference/Kotlin Robot/SVG-composed PNGs, visual diffs, full-screen metrics, visual region metrics, diagnostics, and logs under `build/tmp/vscode-smoke/`.
