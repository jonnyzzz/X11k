...
Build both Docker images with `scripts/run-supervised.sh gradle dockerBuildX11Images`, then run the rough VSCode parity probe explicitly with `-Dx.vscodeParity=true`; it writes Xvfb reference/Kotlin Robot/SVG-composed PNGs, raw SVG/HTML/text diagnostics, SVG layer and HTML per-window preview inventories, visual diffs, full-screen metrics, visual region metrics, diagnostics, and logs under `build/tmp/vscode-smoke/`.
