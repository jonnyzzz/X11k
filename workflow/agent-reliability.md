...
- Failed Gradle and experiment runs also keep their post-failure artifact scans bounded. The native-crash/crash-anchor scan over `build/test-results`, `build/tmp`, and captured logs is wrapped in `GRADLE_DIAGNOSTICS_COMMAND_TIMEOUT_SECONDS` or `EXPERIMENT_DIAGNOSTICS_COMMAND_TIMEOUT_SECONDS` (default 20 seconds), so an expected failing test cannot turn into a long recursive grep over retained IntelliJ/VSCode artifacts.
