...
internal object SvgScreenRenderer {
    ...
    fun json(snapshot: XScreenSnapshot): String =
        buildString {
            ...
            append("""],"window":"${snapshot.pointer.windowIdHex}"},"keyboardState":{"modifierMask":${snapshot.keyboardState.modifierMask},"xkbLatchedModifiers":${snapshot.keyboardState.xkbLatchedModifiers},"xkbLockedModifiers":${snapshot.keyboardState.xkbLockedModifiers},"xkbLatchedGroup":${snapshot.keyboardState.xkbLatchedGroup},"xkbLockedGroup":${snapshot.keyboardState.xkbLockedGroup},"keycodesDown":[""")
            ...
            snapshot.drawings.forEachIndexed { index, drawing ->
                ...
                append(""","renderComposite":""")
                drawing.renderComposite?.let { composite ->
                    append("""{"operation":${composite.operation},"sourcePicture":"0x${composite.sourcePictureId.toUInt().toString(16)}","sourceFormat":${composite.sourceFormat},"sourceKind":"${escapeJson(composite.sourceKind)}","maskPicture":""")
                    composite.maskPictureId?.let { append('"').append("0x${it.toUInt().toString(16)}").append('"') } ?: append("null")
                    append(""","maskFormat":""")
                    composite.maskFormat?.let { append(it) } ?: append("null")
                    append(""","maskKind":""")
                    composite.maskKind?.let { append('"').append(escapeJson(it)).append('"') } ?: append("null")
                    append(""","destinationPicture":"0x${composite.destinationPictureId.toUInt().toString(16)}","destinationFormat":${composite.destinationFormat},"destinationKind":"${escapeJson(composite.destinationKind)}","sourceOrigin":[${composite.sourceX},${composite.sourceY}],"maskOrigin":[${composite.maskX},${composite.maskY}],"destinationOrigin":[${composite.destinationX},${composite.destinationY}],"size":[${composite.width},${composite.height}],"maskDrawable":""")
                    composite.maskDrawableId?.let { append('"').append("0x${it.toUInt().toString(16)}").append('"') } ?: append("null")
                    append(""","maskDrawableGeneration":""")
                    composite.maskDrawableGeneration?.let { append(it) } ?: append("null")
                    append('}')
                } ?: append("null")
            ...
            val drawingRetention = snapshot.drawingRetention
            append("""],"drawingRetention":{"totalRegularCommands":${drawingRetention.totalRegularCommands},"retainedRegularCommands":${drawingRetention.retainedRegularCommands},"regularCommandsComplete":${drawingRetention.totalRegularCommands == drawingRetention.retainedRegularCommands.toLong()},"totalRenderNoOpCommands":${drawingRetention.totalRenderNoOpCommands},"retainedRenderNoOpCommands":${drawingRetention.retainedRenderNoOpCommands},"renderNoOpCommandsComplete":${drawingRetention.totalRenderNoOpCommands == drawingRetention.retainedRenderNoOpCommands.toLong()},"regularCommandBudget":${drawingRetention.regularCommandBudget},"renderNoOpCommandBudget":${drawingRetention.renderNoOpCommandBudget},"combinedCommandBudget":${drawingRetention.regularCommandBudget + drawingRetention.renderNoOpCommandBudget}}""")
            val fillRetention = snapshot.renderFillRetention
            append(""","renderFillRetention":{"totalCommands":${fillRetention.totalCommands},"retainedCommands":${fillRetention.retainedCommands},"commandsComplete":${fillRetention.totalCommands == fillRetention.retainedCommands.toLong()},"totalNoOpCommands":${fillRetention.totalNoOpCommands},"retainedNoOpCommands":${fillRetention.retainedNoOpCommands},"noOpCommandsComplete":${fillRetention.totalNoOpCommands == fillRetention.retainedNoOpCommands.toLong()},"totalRectangles":${fillRetention.totalRectangles},"retainedRectangles":${fillRetention.retainedRectangles},"rectanglesComplete":${fillRetention.totalRectangles == fillRetention.retainedRectangles.toLong()},"resolvedOperationLinks":${fillRetention.resolvedOperationLinks},"operationLinksComplete":${fillRetention.resolvedOperationLinks == fillRetention.retainedCommands},"regularCommandBudget":${fillRetention.regularCommandBudget},"noOpCommandBudget":${fillRetention.noOpCommandBudget},"noOpBudgetShared":true,"combinedCommandBudget":${fillRetention.regularCommandBudget + fillRetention.noOpCommandBudget},"rectangleBudget":${fillRetention.rectangleBudget}}""")
            val compositeRetention = snapshot.renderCompositeRetention
            append(""","renderCompositeRetention":{"totalCommands":${compositeRetention.totalCommands},"retainedCommands":${compositeRetention.retainedCommands},"commandsComplete":${compositeRetention.totalCommands == compositeRetention.retainedCommands.toLong()},"totalNoOpCommands":${compositeRetention.totalNoOpCommands},"retainedNoOpCommands":${compositeRetention.retainedNoOpCommands},"noOpCommandsComplete":${compositeRetention.totalNoOpCommands == compositeRetention.retainedNoOpCommands.toLong()},"resolvedOperationLinks":${compositeRetention.resolvedOperationLinks},"operationLinksComplete":${compositeRetention.resolvedOperationLinks == compositeRetention.retainedCommands},"regularCommandBudget":${compositeRetention.regularCommandBudget},"noOpCommandBudget":${compositeRetention.noOpCommandBudget},"noOpBudgetShared":true,"combinedCommandBudget":${compositeRetention.regularCommandBudget + compositeRetention.noOpCommandBudget}},"renderOperations":${snapshot.renderOperations.size},"renderOperationDetails":[""")
        ...
        with(builder) {
            for (window in snapshot.windows) {
                ...
                svgElement(
                    ...
                    "data-visible-x" to window.visibleX,
                    "data-visible-y" to window.visibleY,
                    "data-visible-width" to window.visibleWidth,
                    "data-visible-height" to window.visibleHeight,
                    "x" to window.x,
                    "y" to window.y,
        ...
        for (window in windows) {
            ...
            builder.svgElement(
                ...
                svgElement(
                    ...
                    "data-visible-x" to window.visibleX - originX,
                    "data-visible-y" to window.visibleY - originY,
                    "data-visible-width" to window.visibleWidth,
                    "data-visible-height" to window.visibleHeight,
