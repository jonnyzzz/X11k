...
internal class X11Connection(
    ...
    private fun xkbSelectEvents(body: ByteArray, majorOpcode: Int) {
        ...
        if ((affectWhich and XXkb.EventIndicatorMapNotify) != 0) {
            state.selectXkbIndicatorMapNotifyInput(
                owner = this,
                clear = (clear and XXkb.EventIndicatorMapNotify) != 0,
                selectAll = (selectAll and XXkb.EventIndicatorMapNotify) != 0,
                affect = details.indicatorMapNotifyAffect,
    ...
    private fun validateXkbSelectEventDetails(
        ...
        fun details32(
            ...
            if (captureIndicatorMapNotify) {
                ...
                indicatorMapNotifySelected = selected
            }
            return true
        }
    ...
    private fun xkbGetState(body: ByteArray, majorOpcode: Int) {
        ...
        reply[13] = keyboardPointerState.normalizedLockedGroup.toByte()
        byteOrder.put16(reply, 16, keyboardPointerState.normalizedLatchedGroup)
    ...
    private fun xkbGetGeometry(body: ByteArray, majorOpcode: Int) {
        ...
        val reply = xkbGeometryReply(geometry, requestedName = name)
        write(reply)
    }

    private fun xkbGeometryReply(geometry: XXkbGeometry?, requestedName: Int): ByteArray {
        val payload = geometry?.payload ?: ByteArray(0)
        val reply = reply(extra = 0, payloadUnits = payload.size / 4)
        byteOrder.put32(reply, 8, geometry?.name ?: requestedName)
        if (geometry != null) {
            reply[12] = 1
    ...
    private fun xkbGetKbdByName(body: ByteArray, majorOpcode: Int) {
        ...
        val componentSpecs = xkbComponentSpecs(body, 8)
        ...
        byteOrder.put16(reply, 12, found)
        byteOrder.put16(reply, 14, reported)
        payload.copyInto(reply, 32)
        write(reply)
    }
    ...
    override fun sendXkbIndicatorMapNotifyEvent(event: XXkbIndicatorMapNotifyEvent) {
        val bytes = ByteArray(32)
        bytes[0] = XXkb.FirstEvent.toByte()
        bytes[1] = XXkb.IndicatorMapNotify.toByte()
        byteOrder.put16(bytes, 2, sequence)
        byteOrder.put32(bytes, 4, event.timestamp)
        bytes[8] = 0
        bytes[9] = 0
