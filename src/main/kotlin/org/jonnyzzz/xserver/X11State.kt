...
internal data class XRenderCompositeExecution(
    ...
    val destinationDrawableId: Int? = null,
    val destinationDrawableGeneration: Long? = null,
    val destinationFormat: Int? = null,
    val sourceDrawableId: Int? = null,
    val sourceDrawableGeneration: Long? = null,
    val sourceFormat: Int? = null,
    val sourceKind: String? = null,
    val maskDrawableId: Int? = null,
    val maskDrawableGeneration: Long? = null,
    val maskFormat: Int? = null,
    val maskKind: String? = null,
...
internal class X11State(
    ...
    private fun removeWindow(id: Int, sendDestroyNotify: Boolean, excludedSink: XEventSink?): XWindowRemoval {
        ...
        val focusDispatches = revertFocusIfWindowRemoved(removed)
    ...
    private fun removeClientResources(resourceIds: Set<Int>, excludedSink: XEventSink?): XResourceRemoval {
        ...
        val saveSetSideEffects = mutableListOf<XSaveSetSideEffect>()
        ...
        for (id in resourceIds) {
            if (id != X11Ids.RootWindow && windows.containsKey(id)) {
                ...
                saveSetSideEffects += removal.saveSetSideEffects
        ...
        return XResourceRemoval(
            ...
            xfixesSelectionNotifyDispatches = xfixesSelectionNotifyDispatches,
            focusDispatches = focusDispatches,
            saveSetSideEffects = saveSetSideEffects,
            pointerUngrabResult = pointerUngrabResult,
    ...
    @Synchronized
    fun selectXkbMapNotifyInput(owner: XEventSink, clear: Boolean, selectAll: Boolean, affect: Int?, selected: Int) {
        var eventMask = xkbMapNotifyInputs[owner] ?: 0
        if (clear) eventMask = 0
        if (selectAll) eventMask = XXkb.AllMapParts
        if (affect != null) eventMask = (eventMask and affect.inv()) or (selected and affect)

        if (eventMask == 0) {
            xkbMapNotifyInputs.remove(owner)
        } else {
            xkbMapNotifyInputs[owner] = eventMask
        }
    }

    @Synchronized
    fun selectXkbControlsNotifyInput(owner: XEventSink, clear: Boolean, selectAll: Boolean, affect: Int?, selected: Int) {
        var eventMask = xkbControlsNotifyInputs[owner] ?: 0
        if (clear) eventMask = 0
        if (selectAll) eventMask = XXkb.AllControlsMask
        if (affect != null) eventMask = (eventMask and affect.inv()) or (selected and affect)
    ...
    fun cursorImageFromPicture(source: XPicture, hotspotX: Int, hotspotY: Int): XCursorImage? {
        ...
        val sourcePixelAt: (x: Int, y: Int) -> Int? = (if (source.alphaMap != 0 || source.hasPictureClip() || source.hasSourceSubwindowClip()) {
    ...
    fun unmapExposeWindows(windowId: Int): List<XWindowExposure> {
        ...
        if (exposedRegions.isEmpty()) return emptyList()
    ...
    private fun exposedWindowsInSubtree(window: XWindow, regions: List<XRectangleCommand>): List<XWindowExposure> {
        ...
        for (child in childrenOf(window.id).asReversed()) {
            ...
            val childRegions = intersectClipLists(ownRegions, listOf(childBounds))
            if (childRegions.isEmpty()) continue
    ...
    @Synchronized
    fun xkbMapNotifyDispatches(event: XXkbMapNotifyEvent): List<XXkbMapNotifyDispatch> {
        if (event.changed == 0) return emptyList()
        return xkbMapNotifyInputs.mapNotNull { (sink, selected) ->
            if ((selected and event.changed) == 0) {
                null
            } else {
                XXkbMapNotifyDispatch(sink = sink, event = event)
            }
        }
    }

    @Synchronized
    fun xkbControlsNotifyDispatches(event: XXkbControlsNotifyEvent): List<XXkbControlsNotifyDispatch> {
        if (event.changedControls == 0) return emptyList()
        return xkbControlsNotifyInputs.mapNotNull { (sink, selected) ->
            if ((selected and event.changedControls) == 0) {
                null
            } else {
                XXkbControlsNotifyDispatch(sink = sink, event = event)
            }
        }
    }
        ...
        return XScreenSnapshot(
            ...
            glxContexts = glxContexts.values.map { context ->
                XGlxContextSnapshot(
                    ...
                    contextMajorVersion = context.contextMajorVersion,
                    contextMinorVersion = context.contextMinorVersion,
            ...
            drawingRetention = XDrawingRetentionSnapshot(
                totalRegularCommands = totalRegularDrawingCommands,
                retainedRegularCommands = drawings.size - retainedRenderNoOpCommands,
                totalRenderNoOpCommands = totalRenderNoOpCommands,
                retainedRenderNoOpCommands = retainedRenderNoOpCommands,
                regularCommandBudget = MaxDrawingCommands,
                renderNoOpCommandBudget = MaxRetainedRenderNoOpCommands,
            ),
            renderFillRetention = XRenderFillRetentionSnapshot(
                ...
                retainedNoOpCommands = drawings.count { it.renderFill != null && !it.framebufferPainted },
                ...
                regularCommandBudget = MaxDrawingCommands,
                noOpCommandBudget = MaxRetainedRenderNoOpCommands,
                rectangleBudget = MaxRetainedRenderFillRectangles,
            ),
            renderCompositeRetention = XRenderCompositeRetentionSnapshot(
                totalCommands = totalRenderCompositeCommands,
                retainedCommands = drawings.count { it.renderComposite != null },
                totalNoOpCommands = totalRenderNoOpComposites,
                retainedNoOpCommands = drawings.count { it.renderComposite != null && !it.framebufferPainted },
                resolvedOperationLinks = drawings.count { drawing ->
                    drawing.renderComposite != null && drawing.renderOperationId?.let { it in retainedRenderOperationIds } == true
                },
                regularCommandBudget = MaxDrawingCommands,
                noOpCommandBudget = MaxRetainedRenderNoOpCommands,
            ),
    ...
    fun updatePicture(
        ...
        alphaMap?.let {
            pictures[id]?.let { picture ->
                picture.alphaMap = it
                picture.alphaMapPicture = if (it == 0) null else pictures[it]
            }
        }
    ...
    fun addGlyphsFromPicture(glyphSetId: Int, source: XPicture, glyphs: List<XPictureGlyph>) {
        val glyphSet = glyphSets[glyphSetId] ?: return
        val sourcePixelAt = if (source.alphaMap != 0 || source.hasPictureClip() || source.hasSourceSubwindowClip()) {
            source.sourcePixelSamplerOptional()
        } else {
            source.sourcePixelSampler()?.let { sampler -> { x: Int, y: Int -> sampler(x, y) } }
        } ?: return
    ...
    private fun drawingPopulationForDrawable(drawableId: Int, generation: Long): XCoreDrawablePaintPopulation {
        ...
        for (drawing in drawings) {
            if (
                ...
                (drawing.renderOperationId == null || drawing.framebufferPainted)
            ) {
    ...
    fun copyPlane(
        ...
        val destinationDepth = drawableDepth(destinationDrawableId)
        val preserveSourcePixel = preserveCoreSourcePixel(function, planeMask, destinationDepth)
        val sourceForeground = if (preserveSourcePixel) corePixelForDepth(foreground, destinationDepth) else foreground
        val sourceBackground = if (preserveSourcePixel) corePixelForDepth(background, destinationDepth) else background
    ...
    fun composite(
        ...
        if (maskPixelAt != null) {
            ...
            } else if (source.hasPictureClip() || source.hasSourceSubwindowClip()) {
            ...
            return when (operation) {
                XRender.OpClear, XRender.OpDisjointClear, XRender.OpConjointClear -> {
                    if (maskFramebuffer == null && maskAlphaAt == null) {
                        if (destinationFramebuffer.fill(
                            destinationX,
                            destinationY,
                            width,
                            height,
        ...
        val sourceSnapshot = sourceFramebuffer.snapshotWithOrigins().takeIf { sourceDrawableId == destinationDrawableId }
        return destinationFramebuffer.compositeGenerated(
            sourceX = sourceX,
            sourceY = sourceY,
            destinationX = destinationX,
            ...
            if (sourceSnapshot != null) {
                if (x in 0 until sourceSnapshot.width && y in 0 until sourceSnapshot.height) {
                    val pixel = decodeDrawablePixelForPictureFormat(
                        sourceSnapshot.pixels[y * sourceSnapshot.width + x],
                        source.format,
                        rawDrawablePixels = sourceSnapshot.rawDrawablePixelAt(x, y),
                    )
                    source.withAlphaMap(pixel, x + 0.5, y + 0.5, sourcePremultiplied = true) ?: 0
                } else {
                    0
                }
            } else {
                sourceFramebuffer.pixelAt(x, y)?.let { pixel ->
                    source.withAlphaMap(
                        decodeDrawablePixelForPictureFormat(pixel, source.format, rawDrawablePixels = sourceFramebuffer.rawDrawablePixelAt(x, y)),
                        x + 0.5,
                        y + 0.5,
                        sourcePremultiplied = true,
                    )
    ...
    fun compositeWithPaintResultAndProvenance(
        ...
        val result = compositeWithPaintResult(
            ...
            image = XImagePixels(0, 0, IntArray(0)),
            painted = false,
        )
        val sourceSnapshot = renderPictureSnapshot(source)
        val maskSnapshot = mask?.let(::renderPictureSnapshot)
        val destinationSnapshot = renderPictureSnapshot(destination)
        return XRenderCompositeExecution(
            ...
            provenance = XRenderOperationProvenance(
                ...
                destinationRegion = XRectangleCommand(destinationX, destinationY, width, height).takeIf { result.painted },
                sourcePopulation = renderDrawablePopulationSnapshot(source),
                maskPopulation = mask?.let(::renderDrawablePopulationSnapshot),
                result = renderResultSnapshot(result.image).takeIf { result.painted },
            ),
            destinationDrawableId = destinationDrawableId,
            ...
            sourceKind = sourceSnapshot.drawableKind,
            maskDrawableId = mask?.drawableId,
            maskDrawableGeneration = mask?.retainedOrLiveDrawableGeneration(),
            maskFormat = mask?.format,
            maskKind = maskSnapshot?.drawableKind,
            destinationKind = destinationSnapshot.drawableKind,
    ...
    fun compositeWithPaintResult(
        ...
        val beforeMetadata = framebuffer.mutationMetadata()
        val image = composite(
            operation = operation,
            source = source,
            mask = mask,
            ...
            framebuffer.restoreMutationMetadata(beforeMetadata)
            return null
        }
        val after = framebuffer.snapshotRegion(destinationX, destinationY, width, height)
        val painted = before != after
        if (!painted) {
            framebuffer.restoreMutationMetadata(beforeMetadata)
        }
        return XRenderImageResult(image, painted = painted)
    }
    ...
    private fun scaledSourcePixelAt(
        ...
        val sourceDrawableId = source.drawableId ?: return null
        val sourceFramebuffer = source.drawableFramebuffer() ?: return null
        val sourceSnapshot = if (sourceDrawableId == snapshotDrawableId) sourceFramebuffer.snapshotWithOrigins() else null
            ...
            if (source.insidePictureClip(x, y)) {
                if (source.alphaMap == 0) {
                    ...
                    if (source.scaleClipsBaseDrawable(sample.first, sample.second, sourceWidth, sourceHeight)) {
                        null
                    } else if (!source.insideSourceSubwindowClip(sourceSubwindowClip, sample.first, sample.second, sourceWidth, sourceHeight)) {
                        null
                    } else {
                        val pixel = if (sourceSnapshot != null) {
                            sourceSnapshot.samplePixelAt(sample.first, sample.second, source.repeat, source.filterName, source.format)
                        } else {
                            sourceFramebuffer.samplePixelAt(sample.first, sample.second, source.repeat, source.filterName, source.format)
                        }
                        pixel
                            ?.let { scaledPixel(it, colorScale, alphaScale) }
                    }
                    ...
                    val pixel = if (sourceSnapshot != null) {
                        source.sampleDrawablePixel(sourceSnapshot, x + 0.5, y + 0.5, source.filterName, sourceSubwindowClip)
                    } else {
                        source.sampleDrawablePixel(sourceFramebuffer, x + 0.5, y + 0.5, source.filterName, sourceSubwindowClip)
                    }
                    pixel
                        ?.let { scaledPixel(it, colorScale, alphaScale) }
                }
    ...
    private fun XFramebuffer.samplePixelAt(x: Double, y: Double, repeat: Int, filterName: String?, format: Int): Int? {
        fun decodedPixelAt(px: Int, py: Int): Int? =
            pixelAt(px, py)?.let { pixel -> decodeDrawablePixelForPictureFormat(pixel, format, rawDrawablePixelAt(px, py)) }
        if (isBilinearFilter(filterName)) {
            return bilinearPixelAt(x, y, repeat, width, height, ::decodedPixelAt)
        }
        if (repeat == XRender.RepeatNone && (x < 0.0 || x >= width || y < 0.0 || y >= height)) {
            return null
        }
        val sampleX = repeatedPixelCoordinate(x, width, repeat) ?: return null
        val sampleY = repeatedPixelCoordinate(y, height, repeat) ?: return null
        return decodedPixelAt(sampleX, sampleY)
    }
    ...
    private fun XFramebufferSnapshot.samplePixelAt(x: Double, y: Double, repeat: Int, filterName: String?, format: Int): Int? {
        fun decodedPixelAt(px: Int, py: Int): Int? =
            pixelAt(px, py)?.let { pixel -> decodeDrawablePixelForPictureFormat(pixel, format, rawDrawablePixelAt(px, py)) }
        if (isBilinearFilter(filterName)) {
            return bilinearPixelAt(x, y, repeat, width, height, ::decodedPixelAt)
    ...
    private fun XPicture.sampleDrawablePixel(
        ...
        val pixel = framebuffer.samplePixelAt(sample.first, sample.second, repeat, effectiveFilterName, format)
            ?: return if (alphaMap == 0) 0 else null
        return withAlphaMap(pixel, sample.first, sample.second, sourcePremultiplied = true)
    }

    private fun XPicture.sampleDrawablePixel(
        snapshot: XFramebufferSnapshot,
        x: Double,
        y: Double,
        effectiveFilterName: String?,
        sourceSubwindowClip: ((Int, Int) -> Boolean)? = null,
        ...
        val pixel = snapshot.samplePixelAt(sample.first, sample.second, repeat, effectiveFilterName, format)
            ?: return if (alphaMap == 0) 0 else null
        return withAlphaMap(pixel, sample.first, sample.second, sourcePremultiplied = true)
    }
    ...
    private fun XPicture.withAlphaMap(pixel: Int, x: Double, y: Double, sourcePremultiplied: Boolean = false): Int? {
        ...
        val alphaFramebuffer = alphaPicture.drawableFramebuffer() ?: return null
    ...
    private fun XPicture.sourcePixelSampler(snapshotDrawableId: Int? = null, premultiplyGradient: Boolean = false): ((x: Int, y: Int) -> Int)? {
        ...
        val framebuffer = drawableFramebuffer() ?: return null
        val sourceSubwindowClip = sourceSubwindowClip()
        val snapshot = if (sourceDrawableId == snapshotDrawableId) framebuffer.snapshotWithOrigins() else null
        if (snapshot != null) {
            return { x, y -> sampleDrawablePixel(snapshot, x + 0.5, y + 0.5, filterName, sourceSubwindowClip) ?: 0 }
        }
    ...
    private fun XPicture.sourcePixelSamplerOptional(
        ...
        return if (snapshot != null) {
            { x, y -> if (insidePictureClip(x, y)) sampleDrawablePixel(snapshot, x + 0.5, y + 0.5, filterName, sourceSubwindowClip) else null }
        } else {
            { x, y -> if (insidePictureClip(x, y)) sampleDrawablePixel(framebuffer, x + 0.5, y + 0.5, filterName, sourceSubwindowClip) else null }
        }
    }
    ...
    private fun XPicture.compositeSourcePixelSamplerOptional(destinationDrawableId: Int): ((x: Int, y: Int) -> Int?)? {
        ...
        val framebuffer = drawableFramebuffer() ?: return null
        val sourceSubwindowClip = sourceSubwindowClip()
        val snapshot = if (sourceDrawableId == destinationDrawableId) framebuffer.snapshotWithOrigins() else null
        if (snapshot != null) {
            return { x, y ->
                if (insidePictureClip(x, y)) {
                    sampleDrawablePixel(snapshot, x + 0.5, y + 0.5, filterName, sourceSubwindowClip)
                } else {
                    null
                }
            }
        }
        return { x, y -> if (insidePictureClip(x, y)) sampleDrawablePixel(framebuffer, x + 0.5, y + 0.5, filterName, sourceSubwindowClip) else null }
    }
    ...
    private fun XPicture.hasSourceSubwindowClip(): Boolean =
        sourceSubwindowClip() != null

    private fun XPicture.sourceSubwindowClip(): ((Int, Int) -> Boolean)? {
        if (subwindowMode == XRender.SubwindowModeIncludeInferiors) return null
        val sourceDrawableId = drawableId ?: return null
        val window = windows[sourceDrawableId] ?: return null
        val childClips = mappedChildClipRectangles(window)
        if (childClips.isEmpty()) return null
        return { x: Int, y: Int -> childClips.none { rectangle -> rectangle.contains(x, y) } }
    }

    private fun XPicture.insideSourceSubwindowClip(
        sourceSubwindowClip: ((Int, Int) -> Boolean)?,
        x: Double,
        y: Double,
        width: Int,
        height: Int,
    ): Boolean {
        if (sourceSubwindowClip == null) return true
        val sampleX = repeatedPixelCoordinate(x, width, repeat) ?: return true
        val sampleY = repeatedPixelCoordinate(y, height, repeat) ?: return true
        return sourceSubwindowClip(sampleX, sampleY)
    }

    private fun XRectangleCommand.contains(x: Int, y: Int): Boolean =
    ...
    private fun decodeDrawablePixelForPictureFormat(pixel: Int, format: Int, rawDrawablePixels: Boolean = true): Int {
    ...
    private fun XPicture.sourcePixelSamplerAt(snapshotDrawableId: Int? = null, filterNameOverride: String? = null): ((x: Double, y: Double) -> Int?)? {
        ...
        val snapshot = if (sourceDrawableId == snapshotDrawableId) framebuffer.snapshotWithOrigins() else null
        return if (snapshot != null) {
            { x, y -> if (insidePictureClip(x, y)) sampleDrawablePixel(snapshot, x, y, effectiveFilterName, sourceSubwindowClip) else null }
        } else {
            { x, y -> if (insidePictureClip(x, y)) sampleDrawablePixel(framebuffer, x, y, effectiveFilterName, sourceSubwindowClip) else null }
        }
    }
    ...
    private fun patternSourcePixelForDepth(pixel: Int, depth: Int, preserveAlpha: Boolean): Int =
        when (depth) {
            1 -> {
                val wirePixel = if ((pixel and 1) != 0 || ((pixel ushr 24) and 0xff) >= 0x80) 1 else 0
    ...
    private fun strokeSource(
        ...
        return { x, y, dashPixel ->
            if (dashPixel != foreground) {
                ...
                )?.let { patternSourcePixelForDepth(it, drawableDepth, preserveAlpha) }
            }
        }
    }
    ...
    fun renderTrapezoids(
        ...
        return framebuffer.changedByRender {
            compositeTrapezoids(
                operation = operation,
    ...
    fun compositeGlyphsInOrder(
        ...
        return destinationFramebuffer.changedByRender {
            ...
            for (placement in placements) {
                ...
                destinationFormat = destination.format,
                    mask = mask,
                    componentMask = XRender.isComponentFormat(glyphSet.format),
                    sourcePremultiplied = source.drawableId != null,
                    sourcePixelAt = sourcePixelAt,
                ) || painted
            }
    ...
    fun compositeGlyphsWithMask(
        ...
        val maskPixels = IntArray(maskWidth * maskHeight) {
            if (XRender.isRgbLikeFormat(maskFormat)) XFramebuffer.opaque(0) else 0
        }
        ...
        return destinationFramebuffer.changedByRender {
            compositeSourceOverMask(
                ...
                mask = temporaryMask,
                destinationFormat = destination.format,
                componentMask = XRender.isComponentFormat(maskFormat),
                sourcePremultiplied = source.drawableId != null,
                sourcePixelAt = sourcePixelAt,
            )
        }
    ...
    fun draw(command: XDrawingCommand) {
        ...
        if (retainedCommand.renderOperationId != null && !retainedCommand.framebufferPainted) {
            retainedRenderNoOpCommands++
            totalRenderNoOpCommands++
        } else {
            totalRegularDrawingCommands++
        }
        if (retainedCommand.renderFill != null && !retainedCommand.framebufferPainted) {
            totalRenderNoOpFills++
        }
        retainedCommand.renderComposite?.let {
            totalRenderCompositeCommands++
            if (!retainedCommand.framebufferPainted) {
                totalRenderNoOpComposites++
            }
        }
        fun removeDrawingAt(index: Int) {
            ...
            if (removed.renderOperationId != null && !removed.framebufferPainted) {
                retainedRenderNoOpCommands--
            }
        }
        if (retainedRenderNoOpCommands > MaxRetainedRenderNoOpCommands) {
            removeDrawingAt(drawings.indexOfFirst { it.renderOperationId != null && !it.framebufferPainted })
        }
        if (drawings.size - retainedRenderNoOpCommands > MaxDrawingCommands) {
            removeDrawingAt(
                drawings.indexOfFirst {
                    it.renderOperationId == null || it.framebufferPainted
                },
            )
        }
    ...
    private fun processSaveSet(saveSet: List<XSaveSetEntry>, resourceIds: Set<Int>): XSaveSetProcessing {
        ...
        val previousCursor = displayedCursorIdentity()
        val sideEffects = mutableListOf<XSaveSetSideEffect>()
        ...
        for (entry in saveSet) {
            ...
            if (entry.map == XSaveSetMap.Map && !window.mapped) {
                ...
                sideEffects += XSaveSetSideEffect.Expose(exposeWindows)
                sideEffects += XSaveSetSideEffect.Focus(unmapResult.focusDispatches)
            }
        }
    ...
    private fun processRetainedSaveSetsForWindowSubtree(windowIds: Set<Int>): XSaveSetProcessing {
        ...
        for (resources in retained) {
            ...
            sideEffects += saveSetProcessing.sideEffects
            xfixesCursorNotifyDispatches += saveSetProcessing.xfixesCursorNotifyDispatches
        }
        return XSaveSetProcessing(
            sideEffects = sideEffects,
    ...
    private var retainedRenderNoOpCommands = 0
    private var totalRegularDrawingCommands = 0L
    private var totalRenderNoOpCommands = 0L
    private var totalRenderFillCommands = 0L
    private var totalRenderNoOpFills = 0L
    private var totalRenderFillRectangles = 0L
    private var totalRenderCompositeCommands = 0L
    private var totalRenderNoOpComposites = 0L
...
private data class XSaveSetProcessing(
    val sideEffects: List<XSaveSetSideEffect> = emptyList(),
    val xfixesCursorNotifyDispatches: List<XXFixesCursorNotifyDispatch> = emptyList(),
)
...
internal data class XPicture(
    ...
    var polyEdge: Int = XRender.DefaultPolyEdge,
    var polyMode: Int = XRender.DefaultPolyMode,
    var dither: Int = 0,
...
internal data class XRenderCompositeCommand(
    val operation: Int,
    val sourcePictureId: Int,
    val sourceFormat: Int,
    val sourceKind: String,
    val maskPictureId: Int?,
    val maskFormat: Int?,
    val maskKind: String?,
    val destinationPictureId: Int,
    val destinationFormat: Int,
    ...
    val width: Int,
    val height: Int,
    val maskDrawableId: Int?,
    val maskDrawableGeneration: Long?,
)
...
internal data class XDrawingCommand(
    ...
    val fillStyle: Int = XGraphicsContext.FillSolid,
    val fillRule: Int = XGraphicsContext.EvenOddRule,
    val tilePixmapId: Int? = null,
    val stipplePixmapId: Int? = null,
    val tileStippleXOrigin: Int = 0,
    val tileStippleYOrigin: Int = 0,
    val arcMode: Int = XGraphicsContext.ArcPieSlice,
    ...
    val renderComposite: XRenderCompositeCommand? = null,
...
internal data class XPutImageMetadata(
    ...
    val rawRowSampleHex: List<List<String>>,
    val decodedRowPixelSampleHex: List<List<String>>,
)
...
internal data class XScreenSnapshot(
    ...
    val drawingRetention: XDrawingRetentionSnapshot,
    val renderFillRetention: XRenderFillRetentionSnapshot,
    val renderCompositeRetention: XRenderCompositeRetentionSnapshot,
...
internal data class XDrawingRetentionSnapshot(
    val totalRegularCommands: Long,
    val retainedRegularCommands: Int,
    val totalRenderNoOpCommands: Long,
    val retainedRenderNoOpCommands: Int,
    val regularCommandBudget: Int,
    val renderNoOpCommandBudget: Int,
)

internal data class XRenderCompositeRetentionSnapshot(
    val totalCommands: Long,
    val retainedCommands: Int,
    val totalNoOpCommands: Long,
    val retainedNoOpCommands: Int,
...
internal data class XGlxContext(
    ...
    val screen: Int,
    val renderType: Int,
    val contextMajorVersion: Int? = null,
    val contextMinorVersion: Int? = null,
...
internal data class XGlxContextSnapshot(
    ...
    val screen: Int,
    val renderType: Int,
    val contextMajorVersion: Int?,
    val contextMinorVersion: Int?,
    val profileMask: Int,
    val direct: Boolean,
