...
class XRenderProtocolTest {
        ...
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                val expectedFormats = listOf(
                    ...
                    XRender.Xrgb32Format,
                    XRender.Abgr32Format,
                    XRender.Xbgr32Format,
                    XRender.Rgb24Format,
                    XRender.Bgr24Format,
                    XRender.X4R4G4B4Format,
                    XRender.X4B4G4R4Format,
                    XRender.X1R5G5B5Format,
                    XRender.X1B5G5R5Format,
                    XRender.A1R5G5B5Format,
                    XRender.A1B5G5R5Format,
                    XRender.R5G6B5Format,
                    XRender.B5G6R5Format,
                    XRender.A4R4G4B4Format,
                    XRender.A4B4G4R4Format,
                    XRender.Bgr32Format,
                    XRender.A2R10G10B10Format,
                    XRender.X2R10G10B10Format,
                    XRender.A2B10G10R10Format,
                    XRender.X2B10G10R10Format,
                )
                assertEquals(expectedFormats.size, u32le(formats, 8))
                ...
                assertEquals(expectedFormats, expectedFormats.indices.map { u32le(formats, 32 + 28 * it) })
                ...
                val xrgb32Offset = 32 + 28 * 3
                assertEquals(XRender.Xrgb32Format, u32le(formats, xrgb32Offset))
                assertEquals(0, u16le(formats, xrgb32Offset + 22))
                val abgr32Offset = 32 + 28 * 4
                assertEquals(XRender.Abgr32Format, u32le(formats, abgr32Offset))
                assertEquals(8, u16le(formats, abgr32Offset + 8))
                assertEquals(24, u16le(formats, abgr32Offset + 16))
                assertEquals(0xff, u16le(formats, abgr32Offset + 22))
                val r5g6b5Offset = 32 + 28 * 14
                assertEquals(XRender.R5G6B5Format, u32le(formats, r5g6b5Offset))
                assertEquals(16, formats[r5g6b5Offset + 5].toInt() and 0xff)
                assertEquals(0x3f, u16le(formats, r5g6b5Offset + 14))
                val bgr24Offset = 32 + 28 * 7
    ...
    @Test
    fun `RENDER CreatePicture accepts Xvfb direct format aliases for matching drawable depths`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 5_000
                setup(socket)
                val depth32Pixmap = PixmapId
                val depth16Pixmap = PixmapId + 1
                val out = socket.getOutputStream()
                out.write(createWindowRequest(WindowId))
                out.write(createPixmapRequest(depth32Pixmap, depth = 32, width = 2, height = 2))
                out.write(renderCreatePicture(PictureId, depth32Pixmap, XRender.Xrgb32Format))
                out.write(renderCreatePicture(PictureId + 1, depth32Pixmap, XRender.Abgr32Format))
                out.write(createPixmapRequest(depth16Pixmap, depth = 16, width = 2, height = 2))
                out.write(renderCreatePicture(PictureId + 2, depth16Pixmap, XRender.R5G6B5Format))
                out.write(renderCreatePicture(PictureId + 3, depth32Pixmap, XRender.R5G6B5Format))
                out.flush()

                assertError(socket.getInputStream(), error = 8, badValue = XRender.R5G6B5Format, sequence = 7, minorOpcode = 4)
                val text = httpGet(server.localPort, "/text.txt")
                assertContains(text, "drawable=0x${depth32Pixmap.toUInt().toString(16)} kind=pixmap")
                assertContains(text, "format=0x${XRender.Xrgb32Format.toUInt().toString(16)}")
                assertContains(text, "format=0x${XRender.Abgr32Format.toUInt().toString(16)}")
                assertContains(text, "format=0x${XRender.R5G6B5Format.toUInt().toString(16)}")
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    @Test
    fun `RENDER Composite samples raw depth32 direct format aliases`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 5_000
                ...
                val xrgbPicture = PixmapPictureId
                val abgrPicture = PixmapPictureId + 1
                val destinationPicture = PictureId
                val out = socket.getOutputStream()
                out.write(createWindowRequest(WindowId))
                out.write(createPixmapRequest(sourcePixmap, depth = 32, width = 2, height = 1))
                ...
                out.write(renderCreatePicture(xrgbPicture, sourcePixmap, XRender.Xrgb32Format))
                out.write(renderCreatePicture(abgrPicture, sourcePixmap, XRender.Abgr32Format))
                out.write(renderComposite(xrgbPicture, destinationPicture, operation = XRender.OpSrc, sourceX = 0, destinationX = 0, destinationY = 0, width = 1, height = 1))
                out.write(renderComposite(abgrPicture, destinationPicture, operation = XRender.OpSrc, sourceX = 1, destinationX = 1, destinationY = 0, width = 1, height = 1))
    ...
    @Test
    fun `RENDER no-alpha direct destinations are opaque for destination-alpha operators`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 5_000
                setup(socket)
                val destinationPixmap = PixmapId
                val xrgbPicture = PixmapPictureId
                ...
                out.write(renderCreatePicture(xrgbPicture, destinationPixmap, XRender.Xrgb32Format))
                out.write(renderFillRectangles(xrgbPicture, x = 0, y = 0, width = 1, height = 1, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0x8000, operation = XRender.OpSrc))
                out.write(renderCreateSolidFill(SolidPictureId, red = 0x0000, green = 0xffff, blue = 0x0000, alpha = 0xffff))
                out.write(renderComposite(SolidPictureId, xrgbPicture, operation = XRender.OpIn, destinationX = 0, destinationY = 0, width = 1, height = 1))
                out.write(renderComposite(xrgbPicture, windowPicture, operation = XRender.OpSrc, destinationX = 0, destinationY = 0, width = 1, height = 1))
    ...
    @Test
    fun `RENDER Composite samples raw depth16 direct format aliases`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 5_000
                ...
                val out = socket.getOutputStream()
                out.write(createWindowRequest(WindowId))
                out.write(createPixmapRequest(sourcePixmap, depth = 16, width = 2, height = 1))
                out.write(createGcRequest(PutImageGcId, sourcePixmap))
                out.write(
                    putImage16OnlyRequest(
                        sourcePixmap,
                        width = 2,
                        height = 1,
                        pixels = intArrayOf(
                ...
                out.write(renderCreatePicture(destinationPicture, WindowId, XRender.Rgb24Format))
                out.write(renderCreatePicture(sourcePicture, sourcePixmap, XRender.R5G6B5Format))
                out.write(renderComposite(sourcePicture, destinationPicture, operation = XRender.OpSrc, destinationX = 0, destinationY = 0, width = 2, height = 1))
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 2, height = 1))
                out.flush()
    ...
    @Test
    fun `RENDER Scale snapshots same drawable source before destination writes`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 2_000
                ...
                out.write(renderScale(PictureId, PictureId, sourceX = 0, destinationX = 1, width = 2, height = 1))
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 3, height = 1))
                out.flush()
    ...
    fun `RENDER picture subwindow mode clips mapped child windows`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                assertEquals(0xffff_0000.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 2, y = 0))
                ...
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 5, y = 0))
    ...
    @Test
    fun `RENDER source picture ClipByChildren clips mapped child window samples`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 2_000
                setup(socket)
                val sourceWindow = WindowId
                val childWindow = WindowId + 1
                val destinationWindow = WindowId + 2
                val sourcePicture = PictureId
                val destinationPicture = PictureId + 1
                val out = socket.getOutputStream()
                out.write(createWindowRequest(sourceWindow, x = 0, y = 0, width = 4, height = 1, borderWidth = 0))
                out.write(mapWindowRequest(sourceWindow))
                out.write(createWindowRequest(childWindow, parent = sourceWindow, x = 1, y = 0, width = 1, height = 1, borderWidth = 0))
                out.write(createWindowRequest(destinationWindow, x = 0, y = 2, width = 4, height = 1, borderWidth = 0))
                out.write(renderCreatePicture(sourcePicture, sourceWindow, XRender.Rgb24Format))
                out.write(renderCreatePicture(destinationPicture, destinationWindow, XRender.Rgb24Format))
                out.write(renderFillRectangles(sourcePicture, x = 0, y = 0, width = 4, height = 1, red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff))
                out.write(renderFillRectangles(destinationPicture, x = 0, y = 0, width = 4, height = 1, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0xffff))
                out.write(mapWindowRequest(childWindow))
                out.write(
                    renderComposite(
                        source = sourcePicture,
                        destination = destinationPicture,
                        width = 4,
                        height = 1,
                        operation = XRender.OpOver,
                        destinationX = 0,
                ...
                out.write(getImageRequest(destinationWindow, x = 0, y = 0, width = 4, height = 1))
                out.write(
                    renderChangePictureAttributes(
                        sourcePicture,
                        XRender.CPSubwindowMode to XRender.SubwindowModeIncludeInferiors,
                ...
                assertExpose(socket.getInputStream(), sourceWindow)
                assertExpose(socket.getInputStream(), childWindow)
                val clippedImage = readReply(socket.getInputStream())
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 4, x = 0, y = 0))
                assertEquals(0xffff_0000.toInt(), pixelAt(clippedImage, imageWidth = 4, x = 1, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 4, x = 2, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 4, x = 3, y = 0))

                val includedImage = readReply(socket.getInputStream())
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 4, x = 0, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 4, x = 1, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 4, x = 2, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 4, x = 3, y = 0))
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    @Test
    fun `RENDER repeated self composite snapshots source before writes`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 5_000
                ...
                out.write(renderFillRectangles(PictureId, x = 2, y = 0, width = 1, height = 1, red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff))
                out.write(renderChangePicture(PictureId, repeat = XRender.RepeatPad))
                out.write(renderComposite(PictureId, PictureId, sourceX = 0, sourceY = 0, destinationX = 1, destinationY = 0, width = 2, height = 1, operation = XRender.OpSrc))
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 3, height = 1))
                out.flush()
    ...
    @Test
    fun `RENDER projective transform horizon samples transparent source pixels`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 5_000
                ...
                out.write(createWindowRequest(WindowId))
                out.write(renderCreatePicture(PictureId, WindowId, XRender.Rgb24Format))
                out.write(renderFillRectangles(PictureId, x = 0, y = 0, width = 1, height = 3, red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff))

                out.write(createPixmapRequest(PixmapId, depth = 24, width = 1, height = 1))
                out.write(renderCreatePicture(PixmapPictureId, PixmapId, XRender.Rgb24Format))
                out.write(renderFillRectangles(PixmapPictureId, x = 0, y = 0, width = 1, height = 1, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0xffff))
                out.write(renderSetPictureTransform(PixmapPictureId, horizonTransform))
                out.write(renderComposite(PixmapPictureId, PictureId, operation = XRender.OpSrc, destinationX = 0, destinationY = 0, width = 1, height = 1))
                ...
                out.write(renderChangePicture(GradientPictureId, repeat = XRender.RepeatPad))
                out.write(renderSetPictureTransform(GradientPictureId, horizonTransform))
                out.write(renderComposite(GradientPictureId, PictureId, operation = XRender.OpSrc, destinationX = 0, destinationY = 1, width = 1, height = 1))
                ...
                out.write(renderChangePicture(GradientNarrowPictureId, repeat = XRender.RepeatPad))
                out.write(renderSetPictureTransform(GradientNarrowPictureId, horizonTransform))
                out.write(renderComposite(GradientNarrowPictureId, PictureId, operation = XRender.OpSrc, destinationX = 0, destinationY = 2, width = 1, height = 1))
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 1, height = 3))
                out.flush()

                val image = readReply(socket.getInputStream())
                assertEquals(0x0000_0000, pixelAt(image, imageWidth = 1, x = 0, y = 0))
                assertEquals(0x0000_0000, pixelAt(image, imageWidth = 1, x = 0, y = 1))
                assertEquals(0x0000_0000, pixelAt(image, imageWidth = 1, x = 0, y = 2))
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    @Test
    fun `RENDER PutImage strip copied through intermediate picture repeats with latest filter and transform`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                setup(socket)
                ...
                val sourcePixels = (topRow + bottomRow).toIntArray()
                val out = socket.getOutputStream()
                out.write(createWindowRequest(WindowId))
                out.write(renderCreatePicture(PictureId, WindowId, XRender.Rgb24Format))
                out.write(createPixmapRequest(sourcePixmap, depth = 32, width = width, height = height))
                ...
                out.write(renderSetPictureFilter(intermediatePicture, "good", values = emptyList()))
                out.write(
                    renderSetPictureTransform(
                        intermediatePicture,
                        listOf(
                ...
                assertPixelRow(image, imageWidth = 8, y = 0, expected = expectedTop)
                assertPixelRow(image, imageWidth = 8, y = 1, expected = expectedBottom)
                assertPixelRow(image, imageWidth = 8, y = 2, expected = expectedTop)
                assertPixelRow(image, imageWidth = 8, y = 3, expected = expectedBottom)

                waitUntil {
                    httpGet(server.localPort, "/text.txt").contains("producerSourcePopulation=0x${sourcePixmap.toUInt().toString(16)}#")
                }
                val text = httpGet(server.localPort, "/text.txt")
                assertContains(text, "source=0x${intermediatePicture.toUInt().toString(16)}/pixmap repeat=normal filter=good")
                assertContains(text, "transform=[0x10000,0x0,0xfffb8000,0x0,0x10000,0x0,0x0,0x0,0x10000]")
                assertContains(text, "sourcePopulation=0x${intermediatePixmap.toUInt().toString(16)}#")
                assertContains(text, "producerSourcePopulation=0x${sourcePixmap.toUInt().toString(16)}#")
                assertContains(text, "putImage=format=2,depth=32,leftPad=0,size=6x2,dataBytes=48,rowStride=24,crc32=")
                assertContains(text, "decoded=[0xff102030,0xff304050,0xff506070")
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    fun `RENDER linear gradient matches Xvfb straight alpha translucent stop interpolation`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                out.write(createWindowRequest(WindowId))
                out.write(renderCreatePicture(PictureId, WindowId, XRender.Rgb24Format))
                out.write(renderFillRectangles(PictureId, x = 0, y = 0, width = 1, height = 1, red = 0xffff, green = 0xffff, blue = 0xffff, alpha = 0xffff))
                out.write(
                    renderCreateLinearGradient(
                        GradientPictureId,
                        p1 = 0 to 0,
                        ...
                        stops = listOf(0, 0x0001_0000),
                        colors = listOf(
                            RenderColor(red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0x0000),
                            RenderColor(red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff),
                        ),
                    ),
                )
                out.write(renderChangePicture(GradientPictureId, repeat = XRender.RepeatPad))
                out.write(renderComposite(GradientPictureId, PictureId, operation = XRender.OpOver, destinationX = 0, destinationY = 0, width = 1, height = 1))
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 1, height = 1))
                out.flush()

                val image = readReply(socket.getInputStream())
                assertEquals(0xffbf_7fbf.toInt(), pixelAt(image, imageWidth = 1, x = 0, y = 0))
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    fun `RENDER trapezoids honor destination poly edge mode`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                out.write(renderChangePictureAttributes(PictureId, XRender.CPPolyEdge to XRender.PolyEdgeSmooth))
                out.write(
                    renderTrapezoidsFixed(
                        SolidPictureId,
                        PictureId,
                        left = 6 * fixedOne,
                        right = 6 * fixedOne + halfPixel,
                        top = 2 * fixedOne,
                        bottom = 6 * fixedOne,
                    ),
                )
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 10, height = 8))
                out.flush()
    ...
    fun `RENDER AddTraps accumulates overlapping A8 coverage`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                out.write(renderChangePictureAttributes(MaskPictureId, XRender.CPPolyEdge to XRender.PolyEdgeSmooth))
                repeat(2) {
                    out.write(
                        renderAddTrapsRaw(
                            MaskPictureId,
    ...
    fun `RENDER AddTraps honors sharp poly edge`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                val image = readReply(socket.getInputStream())
                assertEquals(0xff00_00ff.toInt(), pixelAt(image, imageWidth = 8, x = 4, y = 4))
                assertEquals(0xff00_00ff.toInt(), pixelAt(image, imageWidth = 8, x = 5, y = 4))
            }
            server.close()
            serverThread.join(1_000)
    ...
    @Test
    fun `RENDER Transform accepts bilinear filter id`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                setup(socket)
                ...
                out.write(renderFillRectangles(PixmapPictureId, x = 0, y = 0, width = 1, height = 1, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0xffff))
                out.write(renderFillRectangles(PixmapPictureId, x = 1, y = 0, width = 1, height = 1, red = 0x0000, green = 0xffff, blue = 0x0000, alpha = 0xffff))
                out.write(renderFillRectangles(PixmapPictureId, x = 0, y = 1, width = 1, height = 1, red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff))
                out.write(renderFillRectangles(PixmapPictureId, x = 1, y = 1, width = 1, height = 1, red = 0xffff, green = 0xffff, blue = 0xffff, alpha = 0xffff))
                out.write(
                    renderTransform(
                        PixmapPictureId,
                        PictureId,
                        sourceQuad = listOf(0 to 0, 2 to 0, 2 to 2, 0 to 2),
                        destinationQuad = listOf(0 to 0, 1 to 0, 1 to 1, 0 to 1),
                        filter = 1,
                    ),
                )
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 2, height = 1))
    ...
    @Test
    fun `RENDER Transform accepts advertised filter alias ids`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                setup(socket)
                ...
                out.write(
                    renderTransform(
                        ...
                        PictureId,
                        sourceQuad = listOf(0 to 0, 2 to 0, 2 to 2, 0 to 2),
                        destinationQuad = listOf(1 to 0, 2 to 0, 2 to 1, 1 to 1),
                        filter = 3,
                    ),
                )
                out.write(
                    renderTransform(
                        ...
                        PictureId,
                        sourceQuad = listOf(0 to 0, 2 to 0, 2 to 2, 0 to 2),
                        destinationQuad = listOf(2 to 0, 3 to 0, 3 to 1, 2 to 1),
                        filter = 4,
                    ),
                )
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 4, height = 1))
    ...
    fun `RENDER Transform validates framing operator and pictures`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                out.write(renderTransformRaw(renderTransformBody(SolidPictureId, PictureId, filter = 5)))
                out.write(
                    renderTransformRaw(
                        renderTransformBody(
                            SolidPictureId,
                ...
                assertError(socket.getInputStream(), error = 2, badValue = 5, sequence = 10, minorOpcode = 16)
    ...
    @Test
    fun `RENDER triangles honor destination poly edge mode`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                setup(socket)
                ...
                out.write(
                    renderTrianglesFixed(
                        ...
                        PictureId,
                        points = listOf(
                            4 * fixedOne to 2 * fixedOne,
                            4 * fixedOne + halfPixel to 2 * fixedOne,
                            4 * fixedOne to 6 * fixedOne,
                        ),
                    ),
                )
                out.write(renderChangePictureAttributes(PictureId, XRender.CPPolyEdge to XRender.PolyEdgeSmooth))
                out.write(
                    renderTrianglesFixed(
                        ...
                        PictureId,
                        points = listOf(
                            6 * fixedOne to 2 * fixedOne,
                            6 * fixedOne + halfPixel to 2 * fixedOne,
                            6 * fixedOne to 6 * fixedOne,
                        ),
                    ),
                )
                out.write(getImageRequest(WindowId, x = 0, y = 0, width = 10, height = 8))
                ...
                val image = readReply(socket.getInputStream())
                assertEquals(0xff00_00ff.toInt(), pixelAt(image, imageWidth = 10, x = 4, y = 2))
                assertEquals(0xff7f_0080.toInt(), pixelAt(image, imageWidth = 10, x = 6, y = 2))
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    @Test
    fun `RENDER AddGlyphsFromPicture honors source picture clip rectangles`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            val serverThread = thread(start = true, isDaemon = true) { server.serveForever() }
            Socket("127.0.0.1", server.localPort).use { socket ->
                socket.soTimeout = 2_000
                ...
                out.write(renderSetPictureClipRectangles(MaskPictureId, rectangles = listOf(XRectangleCommand(0, 0, 1, 1))))
                out.write(renderCreateGlyphSet(GlyphSetId, XRender.A8Format))
                out.write(renderAddGlyphsFromPicture(GlyphSetId, MaskPictureId, GlyphId, width = 2, height = 1, xOff = 2))
                out.write(renderCreateSolidFill(SolidPictureId, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0xffff))
                out.write(
                    renderCompositeGlyphs32(
                        source = SolidPictureId,
    ...
    fun `RENDER AddGlyphsFromPicture honors source picture ClipByChildren`() {
        XServer(ServerOptions(port = 0, width = 640, height = 480)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                val sourcePicture = PictureId
                val destinationPicture = PictureId + 10
                val solidPicture = PictureId + 11
                val includedGlyph = GlyphId + 1
                val out = socket.getOutputStream()
                out.write(createWindowRequest(sourceWindow, x = 0, y = 0, width = 4, height = 1, borderWidth = 0))
                out.write(mapWindowRequest(sourceWindow))
                out.write(createWindowRequest(childWindow, parent = sourceWindow, x = 1, y = 0, width = 1, height = 1, borderWidth = 0))
                out.write(createWindowRequest(destinationWindow, x = 0, y = 2, width = 6, height = 1, borderWidth = 0))
                out.write(renderCreatePicture(sourcePicture, sourceWindow, XRender.Rgb24Format))
                out.write(renderCreatePicture(destinationPicture, destinationWindow, XRender.Rgb24Format))
                out.write(renderFillRectangles(sourcePicture, x = 0, y = 0, width = 4, height = 1, red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff))
                out.write(renderFillRectangles(destinationPicture, x = 0, y = 0, width = 6, height = 1, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0xffff))
                out.write(mapWindowRequest(childWindow))
                out.write(renderCreateGlyphSet(GlyphSetId, XRender.A8Format))
                out.write(renderAddGlyphsFromPicture(GlyphSetId, sourcePicture, GlyphId, width = 4, height = 1, xOff = 4))
                out.write(renderCreateSolidFill(solidPicture, red = 0x0000, green = 0x0000, blue = 0xffff, alpha = 0xffff))
                out.write(
                    renderCompositeGlyphs32(
                        source = solidPicture,
                        destination = destinationPicture,
                ...
                out.write(getImageRequest(destinationWindow, x = 0, y = 0, width = 6, height = 1))
                out.write(
                    renderChangePictureAttributes(
                        sourcePicture,
                        XRender.CPSubwindowMode to XRender.SubwindowModeIncludeInferiors,
                ...
                out.write(renderFillRectangles(destinationPicture, x = 0, y = 0, width = 6, height = 1, red = 0xffff, green = 0x0000, blue = 0x0000, alpha = 0xffff))
                out.write(renderAddGlyphsFromPicture(GlyphSetId, sourcePicture, includedGlyph, width = 4, height = 1, xOff = 4))
                out.write(
                    renderCompositeGlyphs32(
                        source = solidPicture,
                        destination = destinationPicture,
                ...
                assertExpose(socket.getInputStream(), sourceWindow)
                assertExpose(socket.getInputStream(), childWindow)
                val clippedImage = readReplySkippingEvents(socket.getInputStream())
                assertEquals(
                    1,
                    clippedImage[0].toInt() and 0xff,
                    "type=${clippedImage[0].toInt() and 0xff} code=${clippedImage[1].toInt() and 0xff} seq=${u16le(clippedImage, 2)} bad=${u32le(clippedImage, 4)} minor=${u16le(clippedImage, 8)} major=${clippedImage[10].toInt() and 0xff}",
                )
                assertEquals(0xffff_0000.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 0, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 1, y = 0))
                assertEquals(0xffff_0000.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 2, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 3, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 4, y = 0))
                assertEquals(0xffff_0000.toInt(), pixelAt(clippedImage, imageWidth = 6, x = 5, y = 0))

                val includedImage = readReplySkippingEvents(socket.getInputStream())
                assertEquals(
                    1,
                    includedImage[0].toInt() and 0xff,
                    "type=${includedImage[0].toInt() and 0xff} code=${includedImage[1].toInt() and 0xff} seq=${u16le(includedImage, 2)} bad=${u32le(includedImage, 4)} minor=${u16le(includedImage, 8)} major=${includedImage[10].toInt() and 0xff}",
                )
                assertEquals(0xffff_0000.toInt(), pixelAt(includedImage, imageWidth = 6, x = 0, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 6, x = 1, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 6, x = 2, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 6, x = 3, y = 0))
                assertEquals(0xff00_00ff.toInt(), pixelAt(includedImage, imageWidth = 6, x = 4, y = 0))
                assertEquals(0xffff_0000.toInt(), pixelAt(includedImage, imageWidth = 6, x = 5, y = 0))
            }
            server.close()
            serverThread.join(1_000)
        }
    ...
    private fun renderTrapezoidsFixed(
        source: Int,
        destination: Int,
        left: Int,
        right: Int,
        ...
        put16le(body, 18, sourceY)
        put32le(body, 20, top)
        put32le(body, 24, bottom)
        putFixedPointRaw(body, 28, left, top)
        putFixedPointRaw(body, 36, left, bottom)
        putFixedPointRaw(body, 44, right, top)
        putFixedPointRaw(body, 52, right, bottom)
        return renderTrapezoidsRaw(body)
    }
    ...
    private fun renderTrianglesFixed(
        source: Int,
        destination: Int,
        points: List<Pair<Int, Int>>,
        operation: Int = XRender.OpOver,
        ...
        points.forEachIndexed { index, point ->
            putFixedPointRaw(body, 20 + index * 8, point.first, point.second)
        }
        return renderTrianglesRaw(body)
    }
    ...
    private fun putImage16OnlyRequest(drawable: Int, width: Int, height: Int, pixels: IntArray, gc: Int = PutImageGcId): ByteArray {
        require(pixels.size == width * height)
        val stride = (width * 2 + 3) and -4
        val data = ByteArray(stride * height)
        for (y in 0 until height) {
            for (x in 0 until width) {
                put16le(data, y * stride + x * 2, pixels[y * width + x])
            }
        }
        val body = ByteArray(20 + data.size)
        put32le(body, 0, drawable)
