    ...
    fun `ListExtensions validates length and returns names with stream recovery`() {
        XServer(ServerOptions(port = 0, width = 120, height = 90)).use { server ->
            ...
            Socket("127.0.0.1", server.localPort).use { socket ->
                ...
                val reply = readReply(socket.getInputStream())
                assertEquals(13, reply[1].toInt() and 0xff)
                assertEquals(2, u16le(reply, 2))
                assertEquals(31, u32le(reply, 4))
                var offset = 32
                ...
                assertEquals(listOf("GLX", "BIG-REQUESTS", "RENDER", "MIT-SHM", "XFIXES", "SHAPE", "XKEYBOARD", "XINERAMA", "XTEST", "XC-MISC", "MIT-SUNDRY-NONSTANDARD", "MIT-SCREEN-SAVER", "SYNC"), names)
            }
            server.close()
            serverThread.join(1_000)
        }
